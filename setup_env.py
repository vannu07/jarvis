...
def create_env_file():
    ...
    app_name = (
        input("App Name [Jarvis AI Assistant]: ").strip() or "Jarvis AI Assistant"
    )
    user_name = input("Your Name [Varnit Kumar]: ").strip() or "Varnit Kumar"
    user_email = (
        input("Your Email [varnitkumar@example.com]: ").strip()
        or "varnitkumar@example.com"
    )
    ...
    face_names = (
        input(
            "Face Recognition Names (comma-separated) [Unknown,Unknown,Varnit Kumar]: "
        ).strip()
        or "Unknown,Unknown,Varnit Kumar"
    )

    env_content.extend([f"FACE_RECOGNITION_NAMES={face_names}", ""])
    ...
    hotwords = (
        input("Hotwords (comma-separated) [jarvis,alexa]: ").strip() or "jarvis,alexa"
    )

    env_content.extend(
        [f"PORCUPINE_ACCESS_KEY={porcupine_key}", f"PORCUPINE_KEYWORDS={hotwords}", ""]
    )
    ...
    env_content.extend(
        [f"WEB_SERVER_HOST={web_host}", f"WEB_SERVER_PORT={web_port}", ""]
    )
